...
+(void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        /*
         ...
         执行class_addMethod  方法 如果的当前类中存在这个方法名(originalSelector)就返回添加失败,为啥失败?因为一个类中不可能有两个相同的方法(完全相同，包括参数),如果该类中没有这个方法名(originalSelector) 就是添加成功
        ...
        /*
         ...
         所以当执行viewWillAppear 这个方法时也就不会执行我们自定义mrc_viewWillAppear，也就得不到我们想要的效果,执行viewWillAppear 的时候同时执行 mrc_viewWillAppear
